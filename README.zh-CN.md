...
**侧边栏** —— 顶部是 **Search sessions（搜索会话）** 输入框，中间是你的项目，底部是 **Add Project（添加项目）**。点项目名只折叠该项目自己的会话列表，切换会话不会改变任何折叠状态。搜索会临时展示所有项目中匹配的会话，清空搜索后各项目恢复原来的折叠状态。每个项目行有 ⋯ 按钮打开[操作菜单](#项目操作菜单)，每个会话行的选项菜单提供重命名、归档和删除。
...
每个会话行的选项菜单提供 **重命名、归档（或取消归档）、删除**。删除操作统一放在菜单中，选择后弹出确认对话框。

**归档** 会立即保存，并从默认列表中隐藏会话。正在运行的进程会继续运行，worktree 和分支原样保留。归档当前会话会清空终端选择。**取消归档** 只把会话放回普通列表，不会启动已经停止的进程。

点击侧边栏的筛选按钮，可选择 **未归档会话、已归档会话、全部会话**，再点 **应用**。显示范围与标题搜索、状态筛选同时生效。**重置** 会把草稿恢复为全部状态、未归档会话；关闭弹窗而不应用会放弃修改。重新打开窗口时恢复默认显示范围，已保存的归档状态不受影响。

当你正在查看其他会话，或窗口不在前台时，收到实时终端输出或退出消息的会话会以 **标题加粗** 表示未读，并向读屏软件提供“有未读输出”说明。图标上的圆点仅表示运行状态。项目名称旁显示当前筛选结果中有未读输出的会话数量。在前台窗口打开相应会话后，标题恢复普通字重。已有未读标记会保存并在重新打开窗口后恢复；历史输出回放不会产生新标记。未读活动在窗口连接期间记录，不代表 agent 已成功完成任务。
