...
点击搜索框右侧的小号灰色漏斗图标，打开筛选表单。在 **会话状态** 下拉框选择 **全部状态、运行中、已完成、已停止、启动中、路径不存在、错误**，然后点击 **应用筛选**。**重置筛选** 会重置表单，关闭弹窗而不应用会放弃修改。图标按钮无边框，筛选生效后漏斗由线框变为实心，悬停可查看当前条件。状态筛选与标题搜索同时生效，结果随会话状态实时更新。筛选会临时展开项目，保留原来的折叠状态和当前终端。没有匹配结果时，可点 **清除筛选** 同时重置搜索和状态条件；重新打开或刷新窗口后，状态筛选恢复为“全部状态”。
...
任意滚动 60 秒内连续点击，且相邻点击间隔不超过 3 秒：达到 32 次解锁双发，达到 48 次解锁三发，之后的连续点击保持已解锁数量。火箭均保持原尺寸，带明亮喷焰和光晕。双发、三发时，每枚火箭下落后独立随机转向，沿直线缓慢行驶 2 秒，再加速驶离窗口，机头始终朝向飞行方向。单发保留原来的 3 秒慢行。点击间隔超过 3 秒就重新计数，下一次点击恢复发射一枚；恰好 3 秒仍算连续。重新打开或刷新窗口也会重置计数。
