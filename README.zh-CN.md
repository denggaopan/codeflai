...
每个会话行的选项菜单提供 **重命名、归档（或取消归档）、删除**，原有的直接删除按钮也可继续使用。
...
当你正在查看其他会话，或窗口不在前台时，收到实时终端输出或退出消息的会话会显示 **有未读输出** 圆点。项目名称旁显示当前筛选结果中有未读输出的会话数量。在前台窗口打开相应会话后，标记清除。已有未读标记会保存并在重新打开窗口后恢复；历史输出回放不会产生新标记。未读活动在窗口连接期间记录，不代表 agent 已成功完成任务。
