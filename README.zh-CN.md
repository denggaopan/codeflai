...
在 Settings（设置）中开启 **显示快捷提示词栏** 后即可使用。此开关默认关闭，选择会在重启后保留。关闭后隐藏底部栏并停用其搜索快捷键，已保存的提示词仍会保留。

终端底部的 **快捷提示词** 栏只显示已星标的提示词，点击即可填入终端当前光标处，再按 **Enter** 发送。点击 **+** 直接填写内容，无需名称；新建提示词默认不星标，可在编辑区或管理列表中点星标加入快捷栏。按钮直接预览内容，放不下的星标项通过 **+N** 入口查看，底栏保持单行且不出现水平滚动条。保存后回到快捷栏，不会自动填入。

点击 **快捷提示词**，或在工作区按 Windows 的 **Ctrl+Shift+P** / macOS 的 **Cmd+Shift+P** 打开搜索，可找到所有提示词，包括未星标的项；使用 **↑↓** 选择、**Enter** 填入、**Esc** 返回终端。右侧管理按钮展开星标、编辑和删除功能，终端在上方保留显示空间。关闭编辑区或切换会话后，未保存的草稿仍会保留，点击 **+** 可以继续编辑；保存或取消后清除草稿。

旧版本保存的内容和开关选择会自动迁移；旧提示词默认不星标，需要手动星标才会出现在底栏。

提示词保存在本机，所有项目和会话共用，重启后仍然保留。已停止的会话也可以管理提示词，恢复会话后才能填入。Agent 会话支持多行提示词；Shell 会话需要支持 bracketed paste 才能填入多行内容，否则会提示使用单行提示词，避免换行被当作执行命令。在未启用 bracketed paste 时，缩进制表符会转为空格。
