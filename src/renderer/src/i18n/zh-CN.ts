import type { Translations } from './en-types'

/**
 * Simplified Chinese dictionary. Typed as `Translations`, so adding a key to en.ts breaks
 * the build here until it is translated — the dictionaries can never silently drift apart.
 * Product names (Codeflai, Claude, Codex, PowerShell, VS Code) and CLI flags stay verbatim.
 */
export const zhCN: Translations = {
  'common.cancel': '取消',
  'common.confirm': '确认',
  'common.delete': '删除',
  'common.remove': '移除',

  'titleBar.settings': '设置',
  'titleBar.launchRocket': 'Codeflai — 发射一枚火箭',
  'titleBar.pinWindow': '窗口置顶',
  'titleBar.unpinWindow': '取消窗口置顶',

  'sidebar.searchSessions': '搜索会话',
  'sidebar.searchTitle': '搜索',
  'sidebar.closeSearch': '关闭搜索',
  'sidebar.clearSearch': '清除',
  'sidebar.sessionStatus': '会话状态',
  'sidebar.filterSessions': '筛选会话',
  'sidebar.filterTitle': '筛选',
  'sidebar.collapseAllProjects': '全部折叠',
  'sidebar.expandAllProjects': '全部展开',
  'sidebar.collapseResults': '折叠搜索/筛选结果',
  'sidebar.expandResults': '展开搜索/筛选结果',
  'sidebar.closeFilters': '关闭筛选',
  'sidebar.resetFilters': '重置',
  'sidebar.applyFilters': '应用',
  'sidebar.allStatuses': '全部状态',
  'sidebar.stoppedStatus': '已停止',
  'sidebar.noMatchingSessions': '没有符合条件的会话',
  'sidebar.clearFilters': '清除筛选',
  'sidebar.dismissNotice': '关闭提示',
  'sidebar.projectOptions': '{project} 的项目选项',
  'sidebar.newSession': '新建会话',
  'sidebar.openInVSCode': '在 VS Code 中打开项目',
  'sidebar.openProjectFolder': '打开项目文件夹',
  'sidebar.openRepository': '打开 Git 仓库',
  'sidebar.removeProject': '从列表中移除',
  'sidebar.removeProjectTitle': '移除项目',
  'sidebar.removeProjectPrompt': '确定将“{project}”从列表中移除吗？不会删除磁盘上的任何内容。',
  'sidebar.removeProjectWithSessionsPrompt': '确定将“{project}”从列表中移除吗？它的 {count} 个会话也会一并移除。不会删除磁盘上的任何内容。',
  'sidebar.addProject': '添加项目',
  'sidebar.ordinarySession': '普通会话',
  'sidebar.deleteSessionTitle': '删除会话',
  'sidebar.sessionOptions': '{title} 的会话选项',
  'sidebar.renameSession': '重命名',
  'sidebar.sessionName': '会话名称',
  'sidebar.saveSessionName': '保存会话名称',
  'sidebar.archiveSession': '归档',
  'sidebar.unarchiveSession': '取消归档',
  'sidebar.archivedSession': '已归档',
  'sidebar.sessionVisibility': '会话显示范围',
  'sidebar.activeSessions': '未归档会话',
  'sidebar.archivedSessions': '已归档会话',
  'sidebar.allSessions': '全部会话',
  'sidebar.unreadOutput': '有未读输出',
  'sidebar.unreadCount': '{count} 个会话有未读输出',
  'sidebar.deleteSessionPrompt': '确定删除“{title}”吗？此操作无法撤销。',
  'sidebar.resizeHandle': '调整侧边栏宽度',

  'addProject.close': '关闭添加项目',
  'addProject.source': '项目来源',
  'addProject.folder': '本地目录',
  'addProject.recent': '历史项目',
  'addProject.clone': '克隆 Git 仓库',
  'addProject.folderHint': '打开此电脑上已有的项目文件夹。',
  'addProject.chooseFolder': '选择项目目录',
  'addProject.recentHint': '重新打开已从项目列表中移除的项目。',
  'addProject.noRecent': '暂无项目列表之外的历史项目。',
  'addProject.removeRecent': '从历史记录中删除 {name}',
  'addProject.removing': '正在删除历史记录...',
  'addProject.removeFailed': '无法删除历史项目：{reason}',
  'addProject.repositoryUrl': 'Git 仓库地址',
  'addProject.urlHint': '支持 HTTPS 和 SSH，如 git@host:owner/repository.git。',
  'addProject.invalidUrl': '请输入包含仓库名称的有效 Git 仓库地址。',
  'addProject.targetDirectory': '目标目录',
  'addProject.directoryPlaceholder': '选择仓库保存位置',
  'addProject.browse': '选择目录',
  'addProject.cloneHint': '将在所选目录下创建与仓库同名的子目录。',
  'addProject.destination': '保存到：{path}',
  'addProject.cloneAction': '克隆并打开',
  'addProject.cloning': '正在克隆仓库，可能需要几分钟…',
  'addProject.opening': '正在打开…',
  'addProject.failed': '无法添加项目：{reason}',

  'launcher.createSession': '创建会话',
  'launcher.newSession': '新建会话',
  'launcher.creatingSession': '正在创建会话…',
  'launcher.close': '关闭启动器',
  'launcher.worktreeVariant': '{kind}（新建 worktree）',
  'launcher.allKindsDisabled': '所有会话类型都已在设置中关闭。',

  'sessionKind.shell': 'Shell',
  'sessionKind.powershell': 'PowerShell',
  'sessionKind.cmd': '命令提示符',
  'sessionKind.claude': 'Claude',
  'sessionKind.codex': 'Codex',
  'sessionKind.gemini': 'Gemini',
  'sessionKind.copilot': 'GitHub Copilot',
  'sessionKind.cursor': 'Cursor',
  'sessionKind.comate': 'Comate',
  'sessionKind.qwen': 'Qwen Code',

  'status.running': '运行中',
  'status.done': '已完成',
  'status.stopped': '点击恢复',
  'status.creating': '启动中…',
  'status.missing': '路径不存在',
  'status.error': '错误',

  'terminal.emptyState': '选择或新建一个会话，在这里查看它的终端。',
  'terminal.restartSession': '重启会话',
  'terminal.bypassWarning': '已启用权限与沙箱绕过',
  'terminal.bypassTooltip':
    '该 agent 以绕过权限与沙箱检查的方式运行：它会直接修改文件、执行命令，不再逐次征求确认。',

  'quickPrompts.title': '快捷提示词',
  'quickPrompts.close': '关闭快捷提示词',
  'quickPrompts.manage': '管理提示词',
  'quickPrompts.keyboardHint': '↑↓ 选择 · Enter 填入 · Esc 关闭',
  'quickPrompts.sharedHint': '本机保存，所有会话共用',
  'quickPrompts.dragHint': '拖动调整顺序',
  'quickPrompts.reorder': '调整{name}的顺序',
  'quickPrompts.reorderHint': '拖动手柄排序，或聚焦手柄后按 ↑↓ 调整',
  'quickPrompts.filteredSortHint': '清空搜索后可调整顺序',
  'quickPrompts.reordered': '已将{name}移至第 {position} 项，共 {count} 项',
  'quickPrompts.reorderFailed': '无法保存新顺序，原有顺序已保留，请重新拖动。',
  'quickPrompts.emptyBar': '在管理中星标提示词，即可显示在这里',
  'quickPrompts.starred': '星标提示词',
  'quickPrompts.moreStarred': '更多星标提示词（{count}）',
  'quickPrompts.star': '星标提示词{name}',
  'quickPrompts.unstar': '取消星标{name}',
  'quickPrompts.showInBar': '星标到快捷栏',
  'quickPrompts.inserted': '已填入 · Enter 发送',
  'quickPrompts.search': '搜索提示词',
  'quickPrompts.empty': '保存常用提示词或命令，下次点击即可填入。',
  'quickPrompts.noResults': '没有匹配的提示词。',
  'quickPrompts.add': '新增提示词',
  'quickPrompts.editTitle': '编辑提示词',
  'quickPrompts.content': '内容',
  'quickPrompts.contentPlaceholder': '输入需要填入的提示词或命令…',
  'quickPrompts.save': '保存提示词',
  'quickPrompts.insert': '填入{name}',
  'quickPrompts.edit': '编辑{name}',
  'quickPrompts.editAction': '编辑',
  'quickPrompts.delete': '删除提示词{name}',
  'quickPrompts.deleteTitle': '删除提示词',
  'quickPrompts.deletePrompt': '确定删除已保存的“{name}”吗？',
  'quickPrompts.stopped': '启动或恢复此会话后即可填入提示词。',
  'quickPrompts.multilineUnavailable': '此会话无法在不提交的情况下填入多行内容，请使用单行提示词。',
  'quickPrompts.invalid': '请填写有效内容，勿超出长度限制或包含终端控制字符。',
  'quickPrompts.saveFailed': '无法保存提示词。编辑内容仍然保留，请重试保存。',
  'quickPrompts.limit': '最多可保存 {count} 条提示词，请编辑或删除已有提示词后再添加。',

  'notice.genericError': '出错了。',
  'notice.dirtyWorktree': 'Worktree 中有 {count} 个文件已改动。请先提交或丢弃这些改动，再删除会话。',

  'settings.title': '设置',
  'settings.close': '关闭设置',
  'settings.launchAtLogin': '开机自动启动',
  'settings.launchAtLoginFailed': '无法修改开机启动设置：{reason}',
  'settings.sessionKinds': '会话类型',
  'settings.sessionKindsHint':
    '关闭的会话类型不会出现在“新建会话”菜单中。开启“新建 worktree”后，该类型会额外提供一个选项，让会话运行在独立的 Git worktree 与同名分支中。',
  'settings.moreSessionKinds': '更多 agent CLI（{count}）',
  'settings.columnEnabled': '启用',
  'settings.columnWorktree': '新建 worktree',
  'settings.enableKind': '启用 {kind}',
  'settings.worktreeForKind': '为 {kind} 新建 worktree',
  'settings.theme': '主题',
  'settings.themeDark': '深色',
  'settings.themeLight': '浅色',
  'settings.language': '语言',
  'settings.showQuickPrompts': '显示快捷提示词栏',
  'settings.version': '版本',
  'settings.versionUnknown': '未知',
  'settings.checkForUpdates': '检查更新',
  'settings.checking': '正在检查更新…',
  'settings.upToDate': 'Codeflai 已是最新版本。',
  'settings.updateAvailable': '新版本 {version} 可用。',
  'settings.updateNoReleases': '尚未发布任何版本。',
  'settings.updateFailed': '检查更新失败：{reason}',
  'settings.updateNow': '立即更新',
  'settings.about': '关于 Codeflai',
  'settings.linkRepository': '项目地址',
  'settings.linkChangelog': '更新日志',
  'settings.linkDownload': '下载地址',

  'update.availableTitle': '新版本 {version} 可用',
  'update.availableBody': 'Codeflai 可以为你下载安装包，待你准备好后再安装。',
  'update.noInstallerBody': '请前往 Releases 页面下载适用于当前平台的版本并手动安装。',
  'update.updateNow': '立即更新',
  'update.later': '稍后更新',
  'update.openDownloadPage': '打开下载页面',
  'update.downloadingTitle': '正在下载版本 {version}',
  'update.progress': '{received} / {total}',
  'update.progressUnknownTotal': '已下载 {received}',
  'update.downloadProgressLabel': '下载进度',
  'update.cancelDownload': '取消',
  'update.readyTitle': '版本 {version} 已可安装',
  'update.readyBody': 'Codeflai 将关闭并运行安装程序。请先保存正在运行的会话中的工作。',
  'update.installNow': '立即安装',
  'update.installingTitle': '正在启动版本 {version} 的安装程序',
  'update.installingBody': 'Codeflai 正在关闭会话并交由安装程序接管。',
  'update.failedTitle': '更新下载失败',
  'update.tryAgain': '重试'
}
