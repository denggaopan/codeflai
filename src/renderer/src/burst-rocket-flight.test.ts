...
describe('planBurstRocketFlight', () => {
  it.each([
    ...
    const flight = planBurstRocketFlight({ viewport, origin })
  ...
  it('keeps the nose tangent to the curved glide', () => {
    const flight = planBurstRocketFlight({ viewport: { width: 1440, height: 900 }, origin })
  ...
  it('separates the two rockets into visibly different glide lanes', () => {
    const viewport = { width: 1440, height: 900 }
    const first = planBurstRocketFlight({ viewport, origin, lane: 0 }).keyframes
    const second = planBurstRocketFlight({ viewport, origin, lane: 1 }).keyframes
    for (let index = 1; index < first.length; index++) {
      expect(decode(first[index].transform).y - decode(second[index].transform).y).toBeCloseTo(72, 1)
    }
  })

  it('does not invert the fall or produce invalid frames in a tiny window', () => {
    const flight = planBurstRocketFlight({ viewport: { width: 120, height: 60 }, origin })
