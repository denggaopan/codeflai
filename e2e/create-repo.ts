...
/**
 * Creates a temporary, disposable Git repository suitable for Codeflai's E2E worktree
 ...
 * E2E runs never collide, and Codeflai's own worktree sequence numbering (scoped per
 * repository root) always starts fresh at `worktree-YYMMDD-1` for a freshly created repo.
 */
export function createRepo(): string {
  const repoPath = mkdtempSync(join(tmpdir(), 'codeflai-e2e-repo-'))
  ...
  git(['config', 'user.email', 'codeflai-e2e@example.com'])
  git(['config', 'user.name', 'Codeflai E2E'])
  git(['config', 'commit.gpgsign', 'false'])

  writeFileSync(join(repoPath, 'README.md'), '# Codeflai E2E fixture repository\n', 'utf8')
  git(['add', 'README.md'])
  ...
  git(['remote', 'add', 'origin', 'https://github.com/codeflai-e2e/fixture.git'])
