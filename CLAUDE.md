...
- **自动关机默认关闭**，开启后每 N 分钟检查一次「有没有会话在跑」，没有就强制关机（倒计时十秒可取消）。开关与频率跨重启保留；两个按钮的 aria-label/title 是完整句子（顶部栏没有正文空间，tooltip 是用户唯一能读到「这会把我的电脑关掉」的地方），改文案要同步 `TitleBar.test.tsx` 与 `e2e/auto-shutdown.spec.ts`。
...
- `e2e/auto-shutdown.spec.ts` 另起实例，只断言开关、下拉与 localStorage：最短可选间隔是**一整分钟**，让 e2e 真等两轮既慢又不稳（实测空闲实例偶发自退）。倒计时、取消、关机请求由 store 单测用假时钟覆盖，真实链路的一次性验证结论记在这里：开启 1m 后第 60 秒弹出对话框，无人操作则第 70 秒发出关机请求。
